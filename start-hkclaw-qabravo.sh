...
PID_FILE="$PROJECT_ROOT/hkclaw-qabravo.pid"
ENV_FILE="$PROJECT_ROOT/.env.agent.qabravo"
NODE_BIN="${NODE_BIN:-}"
...
nohup "$NODE_BIN" "$PROJECT_ROOT/dist/index.js" \
  >> "$PROJECT_ROOT/logs/hkclaw-qabravo.log" \
  2>> "$PROJECT_ROOT/logs/hkclaw-qabravo.error.log" &

echo $! > "$PID_FILE"
echo "hkclaw-qabravo started (PID $!)"
echo "Logs: tail -f $PROJECT_ROOT/logs/hkclaw-qabravo.log"
