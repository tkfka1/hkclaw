 ...
 *
 * HKClaw is Discord-only, so registrations must target Discord channel IDs.
...
import { SERVICE_AGENT_TYPE, SERVICE_ID, STORE_DIR } from '../src/config.js';
...
export async function run(args: string[]): Promise<void> {
  ...
  try {
    db.exec(`ALTER TABLE registered_groups ADD COLUMN service_id TEXT`);
  } catch {
    /* column already exists */
  }
  ...
  db.prepare(
    `INSERT OR REPLACE INTO registered_groups
     (jid, name, folder, trigger_pattern, added_at, agent_config, requires_trigger, is_main, service_id, agent_type, work_dir)
     VALUES (?, ?, ?, ?, ?, NULL, ?, ?, ?, ?, NULL)`,
  ).run(
    parsed.jid,
    parsed.name,
