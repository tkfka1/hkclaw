...
Managed by host-driven watcher.
  ...
  it('persists structured GitLab watch metadata for host-driven watchers', async () => {
    await processTaskIpc(
      {
        type: 'schedule_task',
        prompt: `
...
Managed by host-driven watcher.
    ...
    const tasks = getAllTasks();
    expect(tasks).toHaveLength(1);
    expect(tasks[0].ci_provider).toBe('gitlab');
    expect(tasks[0].ci_metadata).toContain('group/project');
    expect(tasks[0].max_duration_ms).toBe(DEFAULT_WATCH_CI_MAX_DURATION_MS);
  });
